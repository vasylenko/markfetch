...
server.registerTool(
  ...
  async ({ url, savePath }) => {
    try {
      ...
      if (markdownBytes > config.maxBytes) {
        return errorResult(
          "too_large",
          `Markdown ${markdownBytes} bytes > MARKFETCH_MAX_BYTES (${config.maxBytes})`,
        );
      }
      // The file at savePath is only ever the markdown
      // of the URL. Fetch / extraction / size-cap failures all return
      // earlier (above) and never reach this branch, so the file is never
      // written for them. save_failed is its own phase, handled locally.
      if (savePath !== undefined) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Saved ${markdownBytes} bytes to ${savePath}`,
            },
          ],
          isError: false,
        };
