...
### Changed
- `savePath` (MCP tool) and `--output` (CLI) now accept paths that are absolute for the host OS the server runs on. POSIX hosts continue to require a leading `/`; Windows hosts now accept drive-letter (`C:\…` / `C:/…`) or UNC (`\\server\share\…`) roots. Absoluteness is derived at runtime from `node:path` instead of a hard-coded literal slash. The schema rejects POSIX-style absolute paths on Windows (e.g. `/foo`, ambiguous without a drive) and Windows-style paths on POSIX, alongside the existing rejection of relative and tilde-prefixed paths.
- Test driver moved from a shell-expanded glob (`tsx --test tests/*.test.ts`) to a Node script (`scripts/run-tests.mjs`) that enumerates `tests/*.test.ts` from `node:fs` so the same `npm test` command works on Windows (cmd.exe does not expand globs) and POSIX shells.
- CI runs the full test suite on `ubuntu-latest` and `windows-latest` as a matrix.
- Two real-URL e2e tests added against `https://code.claude.com/docs/en/how-claude-code-works` — one for STDOUT via the compiled CLI, one for `savePath` via the MCP tool — both running on every matrix job. Mock-server tests still cover correctness against deterministic fixtures; these add a thin smoke layer against an actual public page for regressions that only surface against real markup (TLS, redirects, gzip, charset quirks).
